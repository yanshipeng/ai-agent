"""RAG v1：检索结果 → Context → 带引用约束的 messages / citations。

==========================================================================
做什么
==========================================================================
1) 调 retrieve(include_text=True) 取 TopK
2) 拼成带 [1][2]… 编号的 Context
3) 加上「只能基于 Context / 不足则澄清 / 必须标引用」的 system prompt
4) 生成 citations[]（响应里给前端/评测用）
...
为什么 citations 由检索结果直接生成，而不是解析模型输出？
  v1 保证「响应当场就有引用列表」；即使模型漏标 [n]，前端仍能展示来源。
  模型正文是否真引用了，交给评测/抽检（verify_ask_rag / eval）。
...
# RAG v1 系统提示（写死，评测短语与此对齐；改语义前先改评测）
RAG_SYSTEM_PROMPT = """你是「稳定性排障」助手。必须严格遵守以下规则：
1. 只能基于用户消息里提供的 Context 回答，禁止使用 Context 之外的知识编造细节。
...
def build_rag_user_message(query: str, hits: list[dict[str, Any]]) -> str:
    """用户侧消息：先 Context 后问题（符合「基于材料作答」的阅读顺序）。"""
    context = build_context_block(hits)
    return (
        f"Context:\n{context}\n\n"
        f"用户问题：{query}\n\n"
        "请严格按系统规则作答，并在需要处标注 [n] 引用。"
    )
